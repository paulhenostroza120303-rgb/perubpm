...
export default async function handler(req, res) {
  ...
  try {
    const { apiKey, target, items, customImg, manualDate } = req.body;
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        results.failed++;
        results.errors.push({ item: item.slug || item.name, error: err.message });
      }
    }
    ...
    return res.status(200).json({ 
      success: true, 
      results,
      message: `Procesados: ${results.success}/${results.total} packs subidos a Firebase`
    });
